...
function IngredientCard({ item, index }: {item: (typeof ingredients)[number];index: number;}) {
  ...
  return (
    <div
      ...
      className={`rounded-2xl border border-border bg-card p-6 flex items-start gap-4 transition-all duration-700 ease-out ${
      isVisible ?
      "opacity-100 translate-y-0" :
      "opacity-0 translate-y-8"} hover:shadow-md hover:-translate-y-1 hover:border-primary/30`
      }>
      
      <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center transition-transform duration-300 group-hover:scale-110">
      ...
      <div className="container mx-auto px-4 max-w-5xl">
        <div
          ...
          <p className="text-muted-foreground font-body text-center max-w-2xl mx-auto mb-12">
            Our formula is built using carefully sourced plant-based extracts
            traditionally studied for their role in prostate health, urinary
            comfort, and hormonal balance.
          </p>
        </div>
